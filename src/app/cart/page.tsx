...
function CartContent() {
  ...
  const getOrderStatusLabel = (status: string) => {
    switch (status) {
      ...
      case "confirmed": return "Confirmed and preparing your order";
  ...
  const currentCartActivity = [
    { title: "Items in cart", description: `${cartItems.length} item${cartItems.length === 1 ? "" : "s"} ready to checkout` },
    { title: "Estimated total", description: formatNaira(totalPrice) },
    { title: "Next step", description: "Proceed to checkout or order via WhatsApp" },
  ];

  function handleWhatsAppOrder() {
    if (!phone) return;
    const message = buildOrderWhatsAppMessage(
      cartItems.map((i) => ({ name: i.product.name, quantity: i.quantity, price: i.product.price })),
      totalPrice,
    );
          ...
          orderHistory.map((order) => (
            <div key={order.id} style={{ padding: "0.95rem 1rem", borderRadius: "1rem", background: "rgba(255,255,255,0.92)", border: "1px solid rgba(0,0,0,0.06)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "center" }}>
                <strong style={{ fontSize: "0.95rem" }}>Order #{order.id.slice(0, 8).toUpperCase()}</strong>
                <span style={{ fontSize: "0.8rem", fontWeight: 700, color: order.status === "delivered" ? "var(--clr-success)" : order.status === "cancelled" ? "var(--clr-chili)" : "var(--clr-saffron)" }}>
                  {order.status}
                </span>
              </div>
              <p style={{ margin: "0.4rem 0 0", color: "var(--clr-muted)", fontSize: "0.9rem" }}>
                {getOrderStatusLabel(order.status)} • {new Date(order.created_at).toLocaleDateString()}
              </p>
    ...
    <div style={{ display: "grid", gap: "0.75rem" }}>
      ...
      {currentCartActivity.map((item) => (
        <div key={item.title} style={{ display: "flex", flexDirection: "column", gap: "0.25rem", padding: "0.95rem 1rem", borderRadius: "1rem", background: "rgba(255,255,255,0.92)" }}>
          <span style={{ fontSize: "0.9rem", fontWeight: 700, color: "var(--clr-bark)" }}>{item.title}</span>
          <span style={{ color: "var(--clr-muted)", fontSize: "0.9rem" }}>{item.description}</span>
        </div>
      ))}
    </div>
  );
            ...
            <div style={{ display: "grid", gap: "1.5rem", maxWidth: "720px", margin: "0 auto" }}>
              ...
              <div style={{ display: "grid", gap: "1rem" }}>
                {Array.from({ length: 3 }).map((_, index) => (
                  <div key={index} style={{ display: "grid", gridTemplateColumns: "80px 1fr", gap: "1rem", alignItems: "center", padding: "1rem", background: "white", borderRadius: "1rem", boxShadow: "0 20px 40px rgba(0,0,0,0.06)" }}>
      ...
      <>
        ...
        <main>
          <div className="container" style={{ padding: "2rem var(--space-md)" }}>
            <h1 style={{ fontFamily: "var(--font-display)", fontSize: "2rem", marginBottom: "1rem" }}>Your Cart</h1>
            ...
            <div className="card" style={{ padding: "1.5rem", marginBottom: "2rem" }}>
              <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "1fr 1fr", alignItems: "stretch" }}>
                <ActivitySection />
                <OrderHistorySection />
              </div>
            </div>
            <div style={{ textAlign: "center", padding: "3rem 1rem", borderRadius: "1rem", background: "rgba(255,255,255,0.98)" }}>
              <p style={{ fontSize: "3rem", marginBottom: "1rem" }}>🛒</p>
              <h2 style={{ fontFamily: "var(--font-display)", marginBottom: "0.75rem" }}>Your cart is empty</h2>
              <p style={{ color: "var(--clr-muted)", marginBottom: "1.5rem" }}>Add a few spices to get started.</p>
    ...
    <>
      ...
      <main>
        <div className="container" style={{ padding: "2rem var(--space-md)" }}>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "2rem", marginBottom: "1rem" }}>Your Cart</h1>
          <p style={{ color: "var(--clr-muted)", marginBottom: "2rem" }}>
            Track current cart activity and recent order progress.
          </p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "2rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {cartItems.map(({ product, quantity }) => (
                <div key={product.id} className="card" style={{ display: "grid", gridTemplateColumns: "auto 1fr auto", gap: "1.25rem", alignItems: "center", padding: "1rem" }}>
                  <div style={{ width: "80px", height: "80px", background: "var(--clr-cream-dark)", borderRadius: "var(--radius-md)", overflow: "hidden", position: "relative" }}>
                    {product.image_url ? (
                      <Image src={product.image_url} alt={product.name} fill loading="lazy" style={{ objectFit: "cover" }} />
                    ) : (
                  ...
                  <div>
                    ...
                    <p style={{ color: "var(--clr-saffron-dark)", fontWeight: 700, fontFamily: "var(--font-display)" }}>{formatNaira(product.price * quantity)}</p>
                    <p style={{ fontSize: "0.8125rem", color: "var(--clr-muted)" }}>{formatNaira(product.price)} each</p>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.5rem" }}>
                    <div style={{ display: "flex", alignItems: "center", border: "2px solid var(--clr-cream-dark)", borderRadius: "var(--radius-md)", overflow: "hidden" }}>
                      <button onClick={() => updateQuantity(product.id, quantity - 1)} style={{ padding: "0.3rem 0.75rem", background: "var(--clr-cream-dark)", border: "none", cursor: "pointer", fontWeight: 600 }}>−</button>
                      <span style={{ padding: "0.3rem 0.75rem", fontWeight: 600, background: "white", minWidth: "2.5rem", textAlign: "center" }}>{quantity}</span>
                      <button onClick={() => updateQuantity(product.id, quantity + 1)} style={{ padding: "0.3rem 0.75rem", background: "var(--clr-cream-dark)", border: "none", cursor: "pointer", fontWeight: 600 }}>+</button>
                    </div>
                    <button onClick={() => removeItem(product.id)} style={{ background: "none", border: "none", color: "var(--clr-chili)", fontSize: "0.8125rem", cursor: "pointer", fontWeight: 500 }}>Remove</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="card" style={{ padding: "1.5rem", alignSelf: "start" }}>
              <h2 style={{ fontFamily: "var(--font-display)", fontSize: "1.25rem", marginBottom: "1.25rem" }}>Order Summary</h2>
              {cartItems.map(({ product, quantity }) => (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                ...
                <button onClick={clearCart} style={{ background: "none", border: "none", color: "var(--clr-muted)", fontSize: "0.875rem", cursor: "pointer", textAlign: "center", marginTop: "0.25rem" }}>
                  Clear cart
                </button>
              </div>
            </div>

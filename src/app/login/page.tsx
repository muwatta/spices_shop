        ...
        <div className="card" style={{ width: '100%', maxWidth: '420px', padding: '2.5rem' }}>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <button type="submit" className="btn btn-primary btn-lg" disabled={loading} style={{ marginTop: '0.5rem' }}>
              {loading ? <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', justifyContent: 'center' }}><span className="spinner" /> Signing in...</span> : 'Sign In'}
            </button>
          </form>

          <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.9rem', color: 'var(--clr-muted)' }}>
            No account?{' '}
            <Link href={`/signup?redirect=${redirect}`} style={{ color: 'var(--clr-saffron-dark)', fontWeight: 600 }}>
              Create one
            </Link>
          </p>
        </div>
